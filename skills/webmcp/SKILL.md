...
- If a previously captured descriptor fails with `RegisteredTool must be an
  ...
  missing tool. Re-fetch the descriptor and re-execute immediately, with no
  delay, several times within the same evaluation — measured on beta Forms, the
  first success came on the 8th immediate attempt. Never report a tool missing
  on this error. Contrast `not-registered`, which needs real elapsed time:
  retry that one from the host in a fresh evaluation, because immediate in-page
  retries consume no wall-clock and produce a false "tool-missing". Do not
...
- Batch two to four dependent calls per evaluator invocation and return a
  ...
  Keep large generation flows incremental so the user gets control back quickly.
- Keep the built-in browser pane visible while tools register. This is the
  ...
  have loaded — a false "denied or failed" is common, and it also aborts a
  batched browser call, so keep navigation out of batches.
